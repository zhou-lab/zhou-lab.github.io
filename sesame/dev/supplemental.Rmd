...
```{r previousversion, echo=FALSE, result="asis"}
library(knitr)
df <- rbind(
...
mask. 
 ...
 - Probe_ID: Illumina probe ID
 - UG: Inf-I unmethylated allele / Inf-II (allele A) green channel measurement
 - UR: Inf-I unmethylated allele / Inf-II (allele A) red channel measurement
 - MG: Inf-I methylated allele (allele B) green channel measurement
 - MR: Inf-I methylated allele (allele B) red channel measurement
 - col: color channel
 - mask: whether the probe should be masked for reporting beta values
...
# Address and Manifest Files

Manifest files describe the probe mapping and chip address. The address files
are subset of manifest files that only contains the chip addresses. The
manifest files can be downloaded from
http://zwdzwd.github.io/InfiniumAnnotation

After downloading, one can build the address file or GRanges object, both from
the downloaded manifest files. The following shows an example,

```{r eval=FALSE}
tsv_path = sesameAnno_download("MM285.mm10.manifest.tsv.gz")
## read manifest to GRanges
manifest = sesameAnno_buildManifestGRanges(tsv_path)
## read manifest to tibble
manifest = sesameAnno_readManifestTSV(tsv_path)
## read manifest to address file (for openSesame or readIDATpair)
...
provide a address file that describes the probe information. You should be able
to obtain the probe information manifest from the Illumina support website.
The address file should be formated as a data frame with four columns
minimally: `Probe_ID`, `M`, `U` and `col`. A optional `mask` column may also be
included as a default mask for the platform. The easiest way to format a
SeSAMe-compatible address file is by following internal address table for a
SeSAMe-supported platform or using the `sesameAnno_buildAddressFile` from a
manifest file. The internal address table can be retrieved with the
`sesameDataGet` function:

```{r}
addr = sesameDataGet("MM285.address")$ordering
```
...
```{r eval = FALSE}
betas = openSesame("IDAT_dir", manifest = addr)
sdf = readIDATpair("your_sample_prefix", manifest = addr) # or one sample
```
...
## Genotyping in VCF format {#SNP}

One can output the allele frequencies and output a VCF file with
genotypes. This requires additional SNP information (ref and alt alleles),
which can be downloaded using the following code:

```{r vcf3, warning=FALSE, message=FALSE}
library(tidyverse)
anno = read_tsv(sesameAnno_download("EPIC.hg38.snp.tsv.gz"))
vcf = formatVCF(sdf, anno)
head(vcf)
```

Note that the VCF combines explicity SNPs and implicit SNPs that alters color
channels of Infinium-I probes. For the REF and ALT slots, we used H and D to
denote `A/C/T` and `A/G/T` respectively.
...
vcf=path_to_vcf)`. See
https://github.com/zhou-lab/InfiniumAnnotationV1/tree/main/Anno/EPIC for more
annotation files, including EPICv2.

```{r vcf4, eval=FALSE}
anno = read_tsv(sesameAnno_download("EPICv2.hg38.snp.tsv.gz"))
head(formatVCF(sdf, anno, vcf="~/Downloads/test_output.vcf"))
```
...
One can plot explicit and Infinium-I-derived SNPs to identify potential
sample swaps.

```{r vcf2, fig.height=5, fig.width=5, message=FALSE}
...
For newer arrays, one need to extract extR and extA from the manifest.
For example,

```{r bsconv2, eval=FALSE}
